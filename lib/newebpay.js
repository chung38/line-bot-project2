  ...
  try {
    ...
    if (e.message?.includes("bad decrypt")) {
      ...
      try {
        ...
        const cipherBuf = Buffer.from(data, "hex");
        // 印出完整內容而不只是前 100 字元——這樣才能看出 JSON 是否有正常收尾（結尾的 }）、
        // 還是在某處被截斷。內容本身安全（訂單編號/金額/交易編號等中繼資料，不含卡號）。
        console.error("🔍 完整原始解密內容（關閉 auto-padding）：", JSON.stringify(rawBuf.toString("utf8")));
        console.error("🔍 密文長度(bytes)：", cipherBuf.length, " 密文最後 16 bytes(hex)：", cipherBuf.slice(-16).toString("hex"));
