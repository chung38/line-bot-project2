...
function replaceMapContents(target, next) {
...
function replaceSetContents(target, next) {
...
// ⚠️ 這幾個 load 函式會被 startPeriodicStateRefresh() 反覆呼叫，不是只在啟動時跑一次。
...
async function loadLang() {
  ...
  replaceMapContents(groupLang, next);
}
async function loadInviter() {
  ...
  replaceMapContents(groupInviter, next);
}
async function loadIndustry() {
  ...
  replaceMapContents(groupIndustry, next);
}
...
async function loadDeletedGroups() {
  ...
  replaceSetContents(deletedGroups, next);
