...
//
// extra 可以帶額外欄位一起寫進封鎖紀錄，目前用來存 groupName。
// 為什麼要在這個時間點存：封鎖之後機器人通常就被移出群組了，
// 那時再呼叫 LINE 的 getGroupSummary() 會失敗，後台就只剩一串看不懂的 gid。
async function deleteGroupSettings(gid, extra = {}) {
