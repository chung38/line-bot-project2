...
// ------------------ In‐Memory State ------------------
const groupLang  = new Map<string,Set<string>>(); // gid → Set<langCode>
const groupOwner = new Map<string,string>();      // gid → uid
...
function makeLangTemplate(gid:string){
  ...
  // Buttons 最多 4 個 action，所以這裡只放前三個語言，剩下用完成／取消 message
  const btns = actions.slice(0,2).concat([
    { type:"postback" as const, label: actions[2].label, data: actions[2].data },
    { type:"postback" as const, label: actions[3].label, data: actions[3].data }
  ]);
  // 最後兩個放在 replyMessage，用文字 prompt 讓使用者輸入「完成」或「取消」
  return {
    type:     "template" as const,
    altText:  "請點擊要接收的語言（可複選／取消），完成後請輸入「完成」或「取消」",
    template: {
      type:    "buttons" as const,
      text:    "點擊要接收的語言（可複選／取消）：",
      actions: btns
    }
  };
}
  ...
  async (req,res)=>{
    ...
    await Promise.all(req.body.events.map(async ev=>{
      ...
      if(ev.type==="message" && ev.message?.type==="text" && gid){
        ...
        if(t==="完成"){
          const langs = [...(groupLang.get(gid)||[])].map(c=>LANGS[c]).join("、")||"（未選語言）";
          return client.replyMessage(ev.replyToken!, {
            type:"text", text:`設定完成，目前接收語言：${langs}`
          });
        }
        if(t==="取消"){
          await saveLang(gid, []);
          return client.replyMessage(ev.replyToken!, {
            type:"text", text:"已取消語言設定"
          });
        }
      }
      // !文宣
      ...
      if(ev.type==="message" && ev.message?.type==="text" && gid){
        ...
        if(["完成","取消","!設定"].includes(txt)||txt.startsWith("!文宣")) return;
