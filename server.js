...
function makeLangTemplate(gid){
  ...
  // Buttons Template 最多 4 個
  const actions = allActions.slice(0,4);
  // 「完成」/「取消」改成訊息由使用者輸入
  return {
    type:     "template",
    altText:  "請點擊要接收的語言，完成後請輸入「完成」或「取消」",
    template: {
      type:    "buttons",
  ...
  async (req,res)=>{
    ...
    await Promise.all(req.body.events.map(async ev=>{
      ...
      if(ev.type==="message"&&ev.message?.type==="text"&&gid){
        ...
        if(t==="完成"){
          const langs = [...(groupLang.get(gid)||[])].map(c=>LANGS[c]).join("、")||"（未選）";
          return client.replyMessage(ev.replyToken,{ type:"text", text:`設定完成，目前：${langs}` });
        }
        if(t==="取消"){
          await saveLang(gid,[]);
